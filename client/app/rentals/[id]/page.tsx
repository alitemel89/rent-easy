...
const RentalDetails = () => {
  ...
  return (
    ...
    <div className="p-8 flex gap-4 mx-8">
      <div className="w-2/3 p-8 shadow-xl rounded-3xl bg-white">
        <h1 className="text-2xl font-bold">{rental.title}</h1>
        <p className="text-gray-500">{rental.description}</p>
        <p className="text-xl font-bold mt-4">Price: ${rental.price} / month</p>
        <p>Bedrooms: {rental.bedrooms}</p>
        <p>Bathrooms: {rental.bathrooms}</p>
        <p>Location: {rental.location}</p>
      </div>
      <div className="w-1/3 p-8 shadow-xl rounded-3xl bg-white">
